...
from helpers import  CameraStream, shutdown, mediapipeTo3dpose, mediapipeToVisibility, PoseStabilizer, get_rot_mediapipe, get_rot_hands, draw_pose, keypoints_to_original, normalize_screen_coordinates, get_rot
...
def main():
    ...
    while True:
        ...
        if results.pose_world_landmarks:        #if any pose was detected
            ...
            visibility = mediapipeToVisibility(results.pose_landmarks.landmark if results.pose_landmarks else results.pose_world_landmarks.landmark)
            ...
            pose_dt = now - last_pose_time
            last_pose_time = now
            pose3d = stabilizer.update(pose3d, visibility, pose_dt, params)
            ...
            if not backend.updatepose(params, pose3d, rots, hand_rots, visibility):
                continue
        else:
            missed_pose_frames += 1
            if missed_pose_frames >= 4:
                stabilizer.reset()
                last_pose_time = time.perf_counter()
